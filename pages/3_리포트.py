...
"제안서(임시)" 탭은 my-report/제안카드.md를 읽기 전용으로 보여준다 — /제안
명령이 만든 카드를 report.card_parser로 파싱하고 report.proposal.build()로
7절을 조립할 뿐, 카드 파일 자체는 이 페이지에서 고치지 않는다.
"""
...
from report import card_parser as CP, proposal as PR, sections as S, to_pdf
...
# 제안서 카드 파일 — 여기서는 읽기만 한다. 카드 자체(제안카드.md)는
# /제안 명령이나 사람이 직접 고치고, 이 페이지는 손대지 않는다.
CARD_PATH = C.ROOT / "my-report" / "제안카드.md"
# "적용" 절의 후보(오늘 내가 내린 결정)를 뽑아 올 판단기준.md — 이것도
# 읽기만 한다.
JUDGMENT_PATH = C.ROOT / "판단기준.md"
...
with tab_proposal:
    st.caption("임시 탭입니다 — my-report/제안카드.md를 읽기 전용으로 보여줍니다. "
               "카드 자체는 여기서 고치지 않습니다.")
    if not CARD_PATH.exists():
        st.info(f"{CARD_PATH} 가 아직 없습니다. /제안 명령으로 먼저 만드십시오.")
    else:
        pcards = CP.parse_file(CARD_PATH)
        judgment_text = (JUDGMENT_PATH.read_text(encoding="utf-8")
                         if JUDGMENT_PATH.exists() else "")
        psecs = PR.build(pcards, st.session_state.proposal_human, judgment_text)
        ...
        with pbody:
            if psec["kind"] == "human":
                filled = bool(psec["body"].strip())
                st.markdown(
                    f'<div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">'
                    f'<div style="font-size:19px;font-weight:700">{psec["title"]}</div>'
                    f'{ui.badge("ok" if filled else "warn", "사람이 쓴다")}</div>',
                    unsafe_allow_html=True)

                candidates = psec.get("candidates")
                if candidates is not None:
                    # "적용" 절의 후보(판단기준.md 최근 "오늘 내가 내린 결정") —
                    # 읽기 전용. 입력창(st.text_area)과 분리된 별도 블록이라
                    # 사람이 여기를 고칠 방법이 없다 — 고르는 것과 쓰는 것만
                    # 아래 입력창에서 한다.
                    st.caption("참고 후보 — 판단기준.md 최근 절 · 읽기 전용")
                    if candidates:
                        cand_body = "\n".join(f"- {c}" for c in candidates)
                        st.markdown(
                            f'<div class="card"><div style="white-space:pre-line;'
                            f'font-size:13.5px;line-height:1.7;color:#94a3b8">'
                            f'{cand_body}</div></div>', unsafe_allow_html=True)
                    else:
                        st.caption("판단기준.md에서 후보를 찾지 못했습니다.")

                new_text = st.text_area(
                    psec["title"], value=psec["body"], height=180,
                    placeholder=psec.get("placeholder", ""),
                    key=f"proposal_ta_{psec['title']}")
                if st.button("저장", key=f"proposal_save_{psec['title']}"):
                    st.session_state.proposal_human[psec["title"]] = new_text
                    bad = S.check_phrasing(new_text)
                    if bad:
                        ui.callout(f"인과를 단정하는 표현이 있습니다: "
                                   f"<b>{', '.join(bad)}</b>. 관측 데이터로는 "
                                   f"인과를 주장할 수 없습니다.")
                    else:
                        st.success("저장했습니다 — 인과 단정 표현 검사 통과.")
                    st.rerun()
            else:
                st.markdown(
                    f'<div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">'
                    f'<div style="font-size:19px;font-weight:700">{psec["title"]}</div>'
                    f'{ui.badge("ok", "자동 생성")}</div>', unsafe_allow_html=True)
                st.markdown(
                    f'<div class="card"><div style="white-space:pre-line;'
                    f'font-size:14px;line-height:1.75">{psec["body"]}</div></div>',
                    unsafe_allow_html=True)
                if psec["phrasing_flags"]:
                    ui.callout(f"자동 생성 문장에 인과를 단정하는 표현이 있습니다: "
